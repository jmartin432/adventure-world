...
def move_hero(world):
    ...
    if isinstance(entity, Empty):
        ...
        if hero.get_health() <= 0:
            print_image('./images/death_by_exhaustion.txt')
            alive = False

    # If the next location is an enemy, go to battle function
    elif isinstance(entity, Enemy):
        print_image('./images/' + entity.get_kind() + '.txt')
    ...
    # If the next location is the treasure
        ...
        print_image('./images/treasure.txt')
        found_treasure = True
...
def move_enemies(world):
    ...
    for enemy in enemies:
        ...
        if isinstance(world[next_location], Empty):
            ...
            print_image('./images/' + enemy.get_kind() + '.txt')
...
def battle(enemy):
    ...
    while ans == 'a':
        ...
        if hero.get_health() <= 0:
            print_image('./images/died_in_battle.txt')
            alive = False
            return enemy
        elif enemy.get_health() <= 0:
            print_image('./images/won_the_battle.txt')
...
answer = answer_question(f'Would you like to go on an adventure, {hero_name.title()}? (y/n) ', ['y', 'n'])
...
if answer == 'y':
    ...
    # Print intro images
    print_image('./images/hero.txt')
    print('\n\n\n')
    print_image('./images/adventure_world.txt')
